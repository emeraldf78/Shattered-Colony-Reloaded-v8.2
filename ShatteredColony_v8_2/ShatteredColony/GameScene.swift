...
class GameScene: SKScene {
    ...
    private func showGraceDepotPopup() {
        ...
        // Show bouncing arrow on depot button
        showDepotButtonArrow()
    ...
    private func showDepotButtonArrow() {
        // Find the depot button in build menu
        guard let depotButton = buildingButtons.first(where: { $0.name == "build_depot" }) else { return }
        
        let arrow = SKLabelNode(fontNamed: "Apple Color Emoji")
        arrow.text = "⬇️"
        arrow.fontSize = 24
        arrow.position = CGPoint(x: depotButton.position.x, y: depotButton.position.y + 35)
        arrow.zPosition = ZPosition.popup
        arrow.name = "graceDepotArrow"
        
        // Bouncing animation
        let moveUp = SKAction.moveBy(x: 0, y: 8, duration: 0.4)
        let moveDown = SKAction.moveBy(x: 0, y: -8, duration: 0.4)
